...
set.seed(123)
N = 100
sample_scores<- rtnorm(N, mean=mu, sd=sigma, lower=a, upper=b)
sample_scores


maxi<-pmax(sample_scores,1)
maxi
mini<-pmin(sample_scores,100)
mini
round(sample_scores,digits=0)
round(maxi,digits=0)
round(mini,digits=0)

uniform_distribution<-runif(sample_scores, min = 0, max = 1)
uniform_distribution
hist(uniform_distribution)

interest<-c("sport", "music", "nature", "literature")
#incorrect
assign(paste(names(sample_scores), collapse=""), interest)
...
score_divides<-cut(sample_scores, 10, letters = NULL)
score_divides
